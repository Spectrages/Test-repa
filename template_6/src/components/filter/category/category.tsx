...
const Category: React.FC<ICategoryProps> = ({ title, cleaningData, setSelectedCleaningType, setVisitFrequencyValue, setTypeOfCleaningValue, isDisableOneTimeCleaning, setSelectedAllergies }) => {
        ...
        if (itemId === AllergyItems.CAT) {
            if (selectedCat) {
                ...
                setSelectedAllergies((prevSelectedAllergies) => {
                    return prevSelectedAllergies.filter((data) => data !== itemId);
                });
            } else {
                setSelectedCat(true);
    ...
    return (
        <div className={styles.category}>
            ...
            <div className={styles.category__main_section}>
                    ...
                    <div key={index}>
                        {title === allergyTitle ? (
                            <input
                                ...
                                onChange={() => { setSelectedCleaningType(item.id), checkedItem(item.id), handleCheckBoxes(item.id) }}
                            />
                        ) : (
                            <input
                                id={item.id}
                        ...
                        <label className={styles.category__label} htmlFor={item.id}>
                            <Text className={styles.category__label_text} weight='medium' size='sm' align='center' color='white' >
                                {item.children}
                            </Text>
                        </label>
                    </div>
